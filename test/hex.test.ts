...
import type { Hex } from "../src/helpers/hex";
import {
  add0x,
  assertIsHexString,
...
describe("isStrictHexString", () => {
  it.each(["0x12345", "0x1234567890abcdef", "0x1234567890ABCDEF", "0x1234567890abcdefABCDEF", "0x1234567890abcdefABCDEF1234567890abcdefABCDEF"])(
    "returns true for a valid hex string",
    (hexString) => {
...
describe("assertIsHexString", () => {
  ...
  it.each([
    ...
    expect(() => assertIsHexString(hexString)).toThrow("Value must be a hexadecimal string.");
  });
});

describe("assertIsStrictHexString", () => {
  it.each(["0x12345", "0x1234567890abcdef", "0x1234567890ABCDEF", "0x1234567890abcdefABCDEF", "0x1234567890abcdefABCDEF1234567890abcdefABCDEF"])(
    "does not throw for a valid hex string",
    (hexString) => {
  ...
  it.each([
    ...
    expect(() => assertIsStrictHexString(hexString)).toThrow('Value must be a hexadecimal string, starting with "0x".');
  });
});
...
describe("isValidHexAddress", () => {
  it.each(["0x0000000000000000000000000000000000000000" as Hex, "0xd8dA6BF26964aF9D7eEd9e03E53415D37aA96045" as Hex])(
    "returns true for a valid prefixed hex address",
    (hexString) => {
      expect(isValidHexAddress(hexString)).toBe(true);
    }
  );

  it.each(["0000000000000000000000000000000000000000", "d8dA6BF26964aF9D7eEd9e03E53415D37aA96045"])(
    "returns false for a valid non-prefixed hex address",
    (hexString) => {
...
describe("getChecksumAddress", () => {
  it("returns the checksum address for a valid hex address", () => {
    expect(getChecksumAddress("0x5aaeb6053f3e94c9b9a09f33669435e7ef1beaed")).toBe("0x5aAeb6053F3E94C9b9A09f33669435E7Ef1BeAed");

    expect(getChecksumAddress("0xfb6916095ca1df60bb79ce92ce3ea74c37c5d359")).toBe("0xfB6916095ca1df60bB79Ce92cE3Ea74c37c5d359");

    expect(getChecksumAddress("0x52908400098527886e0f7030069857d2e4169ee7")).toBe("0x52908400098527886E0F7030069857D2E4169EE7");

    expect(getChecksumAddress("0xde709f2102306220921060314715629080e2fb77")).toBe("0xde709f2102306220921060314715629080e2fb77");

    expect(getChecksumAddress("0x8617e340b3d01fa5f11f306f4090fd50e238070d")).toBe("0x8617E340B3D01FA5F11F306F4090FD50E238070D");

    expect(getChecksumAddress("0x27b1fdb04752bbc536007a920d24acb045561c26")).toBe("0x27b1fdb04752bbc536007a920d24acb045561c26");

    expect(getChecksumAddress("0xdbf03b407c01e7cd3cbea99509d93f8dddc8c6fb")).toBe("0xdbF03B407c01E7cD3CBea99509d93f8DDDC8C6FB");

    expect(getChecksumAddress("0xd1220a0cf47c7b9be7a2e6ba89f429762e7b9adb")).toBe("0xD1220A0cf47c7B9Be7A2E6BA89F429762e7b9aDb");

    expect(getChecksumAddress("0x0000000000000000000000000000000000000000")).toBe("0x0000000000000000000000000000000000000000");
  });
...
describe("getChecksumAddress (memoized)", () => {
  it("memoizes results for repeated calls with the same input", () => {
    ...
    const result1 = getChecksumAddress(address);
  ...
  it("handles different inputs correctly", () => {
    ...
    const result1 = getChecksumAddress(address1);
    const result2 = getChecksumAddress(address2);
...
describe("isValidChecksumAddress", () => {
  ...
  it.each(["0xz" as Hex, "0xD8DA6BF26964AF9D7EED9E03E53415D37AA96045" as Hex, "0xCF5609B003B2776699EEA1233F7C82D5695CC9AA" as Hex])(
    "returns false for an invalid checksum address",
    (hexString) => {
